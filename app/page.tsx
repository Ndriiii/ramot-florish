...
export default function Home() {
    ...
    // Logika Pemfilteran dan Penyortiran Produk
    const filteredAndSortedProducts = PRODUCTS
        // 1. Filter berdasarkan Kategori
        .filter(product => activeCategory === "Semua" ? true : product.category === activeCategory)
        // 2. Sortir berdasarkan urutan alfabet yang dipilih
        .sort((a, b) => {
            if (sortOrder === "asc") {
            ...
            return 0; // "default" membiarkan urutan array asli (berdasarkan id)
        });
...
return (
    <main className="min-h-screen bg-[#EFE8E8] text-[#171717] font-sans selection:bg-green-200" onKeyDown={handleKeyDown}>
        ...
        {/* Hero Section */}
        <section id="home" className="relative flex items-center bg-[#EFE8E8] pt-20 overflow-hidden select-none min-h-[90vh] md:min-h-[95vh]">
            <div className="w-full max-w-7xl mx-auto px-6 md:px-20 relative h-[80vh] md:h-[75vh]">
                
                {/* SLIDE 1 LAYOUT: Desain Berbingkai Diperbesar (Fixed object-cover) */}
                <div className={`absolute inset-0 flex flex-col md:grid md:grid-cols-2 gap-8 md:gap-10 items-center justify-center transition-all duration-700 ease-in-out transform ${currentHeroSlide === 0 ? "opacity-100 translate-x-0 pointer-events-auto z-10" : "opacity-0 translate-x-10 pointer-events-none z-0"}`}>
                    <div className="space-y-4 md:space-y-8 text-center md:text-left mt-8 md:mt-0">
                        <h1 className="text-4xl sm:text-6xl md:text-8xl font-sansita leading-[1.1]">Ramot Florist</h1>
                        <p className="text-sm sm:text-lg md:text-xl text-gray-600 max-w-lg mx-auto md:mx-0 leading-relaxed">
                        ...
                        <div className="flex justify-center md:justify-start pt-2">
                            <button onClick={() => scrollTo("catalog")} className="px-6 md:px-8 py-3 bg-[#171717] text-[#EFE8E8] rounded-full font-semibold shadow-xl active:scale-95 transition">Jelajah Koleksi</button>
                        </div>
                    </div>
                    <div className="flex justify-center w-full pb-10 md:pb-0">
                        <div className="relative w-64 sm:w-md aspect-3/4 md:aspect-4/5 rounded-t-[140px] md:rounded-t-[180px] shadow-2xl overflow-hidden hover:rotate-2 transition-transform duration-500 cursor-pointer shrink-0">
                            <Image src="/Hero.webp" alt="Ramot Florist Storefront" fill className="object-cover" priority sizes="(max-width: 768px) 100vw, 50vw"/>
                        </div>
                    </div>
                </div>

                {/* SLIDE 2 LAYOUT BARU: Kolase Dinamis dan Tumpang Tindih */}
                <div className={`absolute inset-0 flex flex-col md:grid md:grid-cols-2 gap-6 md:gap-10 items-center justify-center transition-all duration-700 ease-in-out transform ${currentHeroSlide === 1 ? "opacity-100 translate-x-0 pointer-events-auto z-10" : "opacity-0 -translate-x-10 pointer-events-none z-0"}`}>
                    <div className="relative order-2 md:order-1 w-full h-full max-w-100 md:max-w-lg lg:max-w-xl mx-auto pb-10 md:pb-0 mt-4 md:mt-0 aspect-4/5 md:aspect-square">
                        {/* Gambar 1: Jumbo (Paling Belakang, Kiri Atas, Besar) */}
                        <div className="absolute top-0 left-0 w-3/5 h-3/5 z-0 group -rotate-2">
                            <Image src="/Papan Jakarta 4.webp" alt="Papan Jumbo" fill className="object-contain drop-shadow-xl p-1 md:p-2 group-hover:scale-105 transition-transform duration-500" sizes="(max-width: 768px) 50vw, 30vw"/>
                        </div>
                        {/* Gambar 2: Jakarta (Kanan Atas, Sedang) */}
                        <div className="absolute top-10 right-0 w-2/5 h-2/5 z-10 group rotate-3">
                            <Image src="/JakartaJumbo.webp" alt="Papan Jakarta" fill className="object-contain drop-shadow-xl p-1 md:p-2 group-hover:scale-105 transition-transform duration-500" sizes="(max-width: 768px) 40vw, 20vw"/>
                        </div>
                        {/* Gambar 3: Mahkota 1 (Kanan Bawah, Sedang) */}
                        <div className="absolute bottom-10 right-10 w-2/5 h-2/5 z-20 group -rotate-1">
                            <Image src="/Mahkota5.webp" alt="Papan Mahkota 1" fill className="object-contain drop-shadow-xl p-1 md:p-2 group-hover:scale-105 transition-transform duration-500" sizes="(max-width: 768px) 40vw, 20vw"/>
                        </div>
                        {/* Gambar 4: Mahkota 3 (Kiri Bawah, Tumpang Tindih, Sedang) */}
                        <div className="absolute bottom-0 left-10 w-2/5 h-2/5 z-30 group rotate-2">
                            <Image src="/Mahkota3.webp" alt="Papan Mahkota 3" fill className="object-contain drop-shadow-xl p-1 md:p-2 group-hover:scale-105 transition-transform duration-500" sizes="(max-width: 768px) 40vw, 20vw"/>
                        </div>
                    </div>
                    <div className="space-y-4 md:space-y-6 text-center md:text-left order-1 md:order-2 md:pl-6 mt-8 md:mt-0 z-40 relative">
                        <span className="text-[10px] md:text-xs font-bold uppercase tracking-widest text-green-800 bg-green-100 px-3 py-1 rounded-full">Koleksi Papan</span>
                        <h2 className="text-3xl sm:text-5xl md:text-6xl font-sansita leading-tight mt-2">Karya Estetik & Modern</h2>
                        ...
                        <div className="flex justify-center md:justify-start pt-2">
                            <button onClick={() => { setActiveCategory("Papan Bunga"); scrollTo("catalog"); }} className="px-6 md:px-8 py-3 border-2 border-[#171717] text-[#171717] rounded-full font-bold hover:bg-[#171717] hover:text-white transition active:scale-95">Lihat Papan Bunga</button>
                        </div>
                    </div>
                </div>

                {/* SLIDE 3 LAYOUT: Editorial Magazine Diperbesar */}
                <div className={`absolute inset-0 flex flex-col md:flex-row gap-4 md:gap-10 items-center justify-center transition-all duration-700 ease-in-out transform ${currentHeroSlide === 2 ? "opacity-100 translate-y-0 pointer-events-auto z-10" : "opacity-0 translate-y-10 pointer-events-none z-0"}`}>
                    <div className="hidden md:block w-1/3 lg:w-[28%] relative aspect-3/4">
                        <Image src="/Standing 1.webp" alt="Fresh Flower" fill className="object-contain drop-shadow-2xl p-2" sizes="25vw"/>
                    </div>
                    <div className="w-full md:w-1/3 lg:w-2/5 text-center space-y-4 md:space-y-6 px-4 mt-8 md:mt-0">
                        <span className="text-[10px] md:text-xs font-bold uppercase tracking-widest text-red-800 bg-red-100 px-3 py-1 rounded-full">Layanan Bunga</span>
                        <h2 className="text-3xl sm:text-5xl md:text-6xl font-sansita leading-tight">Khidmat & Cepat</h2>
                        ...
                        <div className="flex justify-center gap-3 pt-2">
                            <button onClick={() => { setActiveCategory("Bunga Salib"); scrollTo("catalog"); }} className="px-4 md:px-6 py-2.5 md:py-3 bg-[#171717] text-white text-xs md:text-sm font-semibold rounded-full shadow-md active:scale-95 transition">Bunga Salib</button>
                            <button onClick={() => { setActiveCategory("Standing Flower"); scrollTo("catalog"); }} className="px-4 md:px-6 py-2.5 md:py-3 bg-white border border-gray-300 text-xs md:text-sm font-semibold rounded-full shadow-sm active:scale-95 transition">Standing Flower</button>
                        </div>
                    </div>
                    <div className="w-3/4 sm:w-2/3 md:w-1/3 lg:w-[28%] relative aspect-4/5 md:aspect-3/4 pb-10 md:pb-0">
                        <Image src="/Salib.webp" alt="Bunga Salib Ucapan" fill className="object-contain drop-shadow-2xl p-2" sizes="(max-width: 768px) 70vw, 25vw"/>
                    </div>
                </div>
            ...
            <div className="absolute bottom-4 md:bottom-6 left-1/2 -translate-x-1/2 flex gap-3 z-30">
                {[0, 1, 2].map((idx) => (
                    <button key={idx} onClick={() => setCurrentHeroSlide(idx)} className={`w-2.5 h-2.5 md:w-3 md:h-3 rounded-full transition-all duration-300 ${idx === currentHeroSlide ? "bg-[#171717] w-6 md:w-8" : "bg-gray-400/50 hover:bg-gray-600"}`} aria-label={`Buka slide ${idx + 1}`} />
                ))}
            </div>
        </section>
        ...
        <section id="catalog" className="relative py-24 overflow-hidden bg-[#E5DEDE]">
            <div className="relative z-10 max-w-7xl mx-auto px-4 md:px-8">
                ...
                <div className="flex flex-col md:flex-row justify-center items-center gap-4 mt-10 mb-12">
                    ...
                    {/* Pembatas Vertikal (hanya muncul di PC) */}
                    <div className="hidden md:block h-8 w-px bg-gray-300 mx-2"></div>

                    {/* Filter Sorting Alphabet */}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 md:gap-6">
                    {/* Menggunakan filteredAndSortedProducts yang sudah melewati logika sorting */}
...
// 4. PRODUCT CARD (GAMBAR TRANSPARAN MELAYANG TANPA BG)
